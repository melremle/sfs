$(function () {
    ...
    tblMyDrive = $("#tbl-drive")?.DataTable({
        ajax: {
            url: baseURL + '/api/file/my-drive?id=' + oid,
            method: 'get',
            dataSrc: function (e) {
                return e
            }
        ...
        createdRow: function (r, d, i) {
            ...
            r.addEventListener('click', f => {
                ...
                $("#shared-to-container .shared-to").remove()

                $.ajax({
                    url: baseURL + '/api/getFileDetails.php?fid=' + d.ID,
                    method: 'get',
                    contentType: false,
                    processData: false,
                    dataType: 'json',
                        ...
                        if (res.length > 0) {
                            if (res[0].isshared == 1) {
                                res.forEach((r, i) => {
                                    if (i < 5) {
                                        const avatar = r.pic
                                        let fAvatar = '<img src="' + baseURL + '/files/logos/' + avatar + '" class="rounded-circle" width="50" alt="" loading="lazy" title="' + r.fullname + '" />'
                                        $('<div class="d-flex align-items-center justify-content-center px-2 py-1 shared-to" >\
                                            <div class="d-flex flex-column align-items-center justify-content-center">\
                                                '+ fAvatar + '\
                                            </div>\
                                        </div>').appendTo('#shared-to-container')
                                    } else if (i = (res.length - 1)) {
                                        const count = (res.length - 5)
                                        $('<div class="rounded-circle border-1">\
                                            +'+ count + ' more\
                                        </div>')
                                    }
                                })
                                ...
                                $("#shared-to-container .shared-to").remove()
                                $('<div class="d-flex align-items-center justify-content-center px-2 py-0 shared-to" >\
                                        <small class="text-danger p-0 m-0">Not Shared</small>\
                                    </div>').appendTo('#shared-to-container')
                            }
                            const size = res[0].size / (1024 * 1024)
                            const uploadedOn = moment(new Date(res[0].created)).format('MMM DD YYYY')
                            const modifiedOn = res[0].created == null ? 'Never' : moment(new Date(res[0].created)).format('MMM DD YYYY')
                            $("#detailsModal #size").text(size.toFixed(2) + ' MiB')
                            $("#detailsModal #type").text(res[0].ftype)
                            $("#detailsModal #owner").text(res[0].owner)
                            $("#detailsModal #uploaded").text(uploadedOn)
                            $("#detailsModal #modified").text(modifiedOn)
        ...
        $.ajax({
            ...
            success: function (r) {
                ...
                r.forEach(res => {
                    ...
                    let fAvatar = '<img src="' + baseURL + '/files/logos/' + avatar + '" class="rounded-circle" width="45" alt="" loading="lazy" title="' + res.office + '" />'
    ...
    function formatDisplay(repo) {
        ...
        let fAvatar = '<img src="' + baseURL + '/files/logos/' + avatar + '" class="rounded-circle" width="50" alt="" loading="lazy" title="' + repo.office + '" />'
    ...
    $("#link-archive").click(function () {
        ...
        closeContext()
        $.ajax({
            url: baseURL + '/api/file/move-archive',
            method: 'post',
            data,
            contentType: false,
            processData: false,
    ...
    $("#link-delete").click(function () {
        ...
        closeContext()
        $.ajax({
            url: baseURL + '/api/file/move-trash',
            method: 'post',
            data,
            contentType: false,
            processData: false,
