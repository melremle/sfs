$(function () {
    ...
    tblMyDrive = $("#tbl-drive")?.DataTable({
        ajax: {
            url: baseURL + '/api/file/shared-with-me',
            method: 'get',
            dataSrc: function (e) {
                return e
            }
        ...
        columns: [
            {
                ...
                render: function (d, t, r) {
                    ...
                    const fname = r.fullname == null ? r.username : r.fullname
                    if (avatar == null) {
                        fAvatar = '<div data-mdb-toggle="tooltip" title="' + fname + '"class="avatar-letter-' + name.toLowerCase() + '">' + name.toUpperCase() + '</div>'
                    }
                    const markup = '\
            ...
            },
            {
                data: null,
                render: function (d, t, r) {
                    const uploadedOn = moment(new Date(r.sharedon)).format('MMM DD YYYY')
                    const formattedDate = uploadedOn
                    return '<span class="text-secondary text-xs font-weight-bold">' + formattedDate + '</span>'
                }
            }
    ...
    $('#encKeyModal').on("shown.bs.modal", function () {
        $("#key-container #password").focus()
    });

    let keyVal = ""

    $("#key-container #password").on('input', function () {
        keyVal = this.value
        if (keyVal.trim().length == 0) {
            $("#verify-key-btn").attr('disabled', 'disabled')
        } else {
            $("#verify-key-btn").removeAttr('disabled')
        }
    })

    $("#verify-key-btn").click(function () {
        const data = new FormData()
        data.append('key', keyVal)
        data.append('fid', dataId)
        $.ajax({
            url: baseURL + '/api/file/key',
            method: 'post',
            data,
            contentType: false,
            processData: false,
            ...
            success: function (res) {
                if (res.success == false) {
                    ...
                    let a = $("<a>")
                        .attr("href", baseURL + '/api/file/download-now?fid=' + dataId)
                        .appendTo("body");
                    a[0].click();
                    a.remove();
                    $("#key-container #password").val('')
                    keyVal = ""
                    $("#encKeyModal").modal('hide')
